...
{
    public class NoteForItemController : NoteController
    {
        protected override NoteData CreateData(Note t, int id)
        {
            return CreateDataFor(t, id);
        }

        protected override NoteData CreateData(Note source, int id, string parentType)
        {
            var data = CreateDataFor(source, id, null, parentType);
            data.ParentType = parentType;
            NoteData.SetParent(db, parentType, data);
            // var project = ItemData<Task, Item>.GetProject(data.Parent, db);
