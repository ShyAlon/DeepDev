...
.service('ui', ['$http', '$q', '$rootScope', '$location', '$window', 'server', 'local', 'rnd', 'default_definition', 'post_script_functions',
    function ($http, $q, $rootScope, $location, $window, server, local, rnd, default_definition, post_script_functions) {

    var configureDatetimepickerInternal = function (scope) {
        /* date picker */
        ...
        scope.formats = ['longDate', 'dd-MMMM-yyyy', 'yyyy/MM/dd', 'shortDate'];
        scope.format = scope.formats[0];
    };

    var configureWizardInternal = function (scope, steps, done) {
        scope.steps = steps;
        scope.step = 0;
        ...
        scope.wizardClass = function (step) {
            if (scope.step === step) {
                return 'active-step';
    ...
    var initSequenceDiagramnternal = function (scope, realScope) {
        if (!realScope) {
            realScope = scope;
        }
        if (!scope) {
        ...
        if (scope == realScope) {
            // console.log(realScope);
            realScope.$watch('definition', function () {
                scope.draw();
            });
        } else {
            // console.log(realScope);
            realScope.$watch('scope.definition', function () {
                scope.draw();
            });
        }
        ...
        scope.draw = function () {
            ...
            for (var i in lines) {
                ...
                if (/^([^-]*(-+[^-]+)*)(-->|->)(.*):(.*)$/.exec(line) != null) {
                    ...
                    else if (position == 'left of') { dx = -scope.margin - width; }
                    else if (position == 'over') { dx = -width / 2; }
                    scope.notes.push({
                        x: user.x + /*scope.user_width*/ width + dx,
                        y: scope.height,
                        text: text
                    });
        ...
        scope.getUser = function (name, alias) {
            ...
            var user = {
                x: Math.max(1, scope.users.length * 250 + 1),
                y: 1,
                name: name,
                alias: alias,
                first: scope.users.length == 0
            };
    ...
    return {
        configureDatetimepicker: function (scope) {
            configureDatetimepickerInternal(scope);
        },

        configureWizard: function (scope, done, steps) {
            configureWizardInternal(scope, done, steps);
        },

        initSequenceDiagram: function (scope, realScope) {
            initSequenceDiagramnternal(scope, realScope);
        }
    };
}]).service('rnd', function () {
    var seed = 123;
    return {
        reset: function () {
            seed = 123;
...
B->A: RequestCreated\n\
A->User: Done').value('post_script_functions',
'/SEED 120 def\n\
/MOD {\n\
